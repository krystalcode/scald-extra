...
renderLibrary: function(data, editor) {
  ...
  var dndStatus = {
    search: library_wrapper.hasClass('search-on'),
  ...
  library_wrapper.find('.scald-anchor').click(function() {
    ...
    if (library_wrapper.hasClass('library-on')) {
      library_wrapper.animate({
        width: Drupal.settings.scald_extra.dnd_menu_width + 'px'
      ...
      scald_library.animate({
        width: '0'
      }, { duration: 490, queue: false });
      scald_filters.animate({
        width: '0'
      }, { duration: 490, queue: false });
    } else {
      library_wrapper.animate({
        ...
        library_wrapper.toggleClass('animate');
        scald_library.animate({
          width: Drupal.settings.scald_extra.dnd_content_width + 'px'
        }, { duration: 510, queue: false });
        scald_filters.animate({
          width: Drupal.settings.scald_extra.dnd_content_width + 'px'
        }, { duration: 510, queue: false });
      }, done: function() {
  ...
  library_wrapper.find('.pager a').click(function(e) {
    e.preventDefault();
    library_wrapper.find('.pager').hide();
    library_wrapper.find('.pager-spinner').addClass('fa-spin').css('display', 'block').show();
    $.getJSON(this.href, function(data) {
      Drupal.behaviors.dndLibrary.renderLibrary.call(library_wrapper.get(0), data, $(editor));
      library_wrapper.find('.pager').show();
      library_wrapper.find('.pager-spinner').hide().removeClass('fa-spin');
    });
    return false;
  });
  ...
  library_wrapper
    .find('.view-filters .views-submit-button')
    ...
    .click(function(e) {
      ...
      submit
        ...
        .append('<span style="margin-left:5px" class="fa fa-spinner fa-spin"></spin>');
      ...
      library_wrapper.find('.view-filters form').ajaxSubmit({
        ...
        'success' : function(data) {
          ...
          library_wrapper.find('.toggle').click();
